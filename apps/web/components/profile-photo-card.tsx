...
export function ProfilePhotoCard({
  ...
  const shellClasses = cn(compact ? "w-36" : "w-full max-w-[20rem]", className)

  if (!resolvedPhotoUrl) {
    return (
      <div className={shellClasses}>
        <AspectRatio ratio={9 / 11}>
          <div className="flex h-full items-center justify-center rounded-[24px] border border-dashed border-white/10 bg-black/20">
  ...
  if (!unlocked && visibility === "family") {
    return (
      <div className={shellClasses}>
        <AspectRatio ratio={9 / 11}>
          <div className="relative h-full overflow-hidden rounded-[24px] border border-white/15 bg-[radial-gradient(circle_at_top,#5f4611_0%,#1b140a_45%,#0a0a0c_100%)]">
      ...
      <AspectRatio ratio={9 / 11}>
        <div
          className={cn(
            ...
            unlocked && "border-primary/20 shadow-[0_20px_60px_rgba(0,0,0,0.22)]",
          )}
        >
          <img
          ...
          <div
            className={cn(
              "absolute inset-0",
              unlocked
                ? "bg-gradient-to-t from-[#080809]/30 via-transparent to-transparent"
                : "bg-gradient-to-t from-[#080809]/85 via-[#080809]/35 to-transparent",
            )}
          />
          ...
          {unlocked ? (
            <div className="absolute inset-x-0 bottom-0 p-4">
              <div className="rounded-2xl border border-white/10 bg-black/25 px-4 py-3 backdrop-blur">
                ...
                <p className="mt-1 text-[11px] uppercase tracking-[0.2em] text-muted-foreground">{copy.body}</p>
              </div>
            </div>
          ) : (
